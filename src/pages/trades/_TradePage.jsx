    ...
    <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(255,255,255,0.96)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${B.border}` }}>
      <div style={{ maxWidth: 1140, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 62 }}>
        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <span style={{ fontWeight: 800, fontSize: 16, color: B.navy }}>text<span style={{ color: B.blue }}>reminder</span></span>
        </Link>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Link to="/login" style={{ fontSize: 14, fontWeight: 500, color: B.muted, textDecoration: 'none', padding: '8px 12px' }}>Log in</Link>
          <button onClick={onSignup} style={{ background: B.blue, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
            Start free
          </button>
        </div>
      </div>
  ...
  return (
    <footer style={{ background: B.navy, padding: '40px 20px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 9 }}>
          <span style={{ fontWeight: 800, fontSize: 15, color: '#fff' }}>text<span style={{ color: '#93c5fd' }}>reminder</span></span>
        </Link>
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: B.text, background: B.white }}>
      ...
      <section style={{ background: B.navy, padding: '80px 20px 90px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ fontSize: 42, marginBottom: 20 }}>{emoji}</div>
          ...
          <button onClick={handleSignup}
            style={{ background: B.blue, color: '#fff', border: 'none', borderRadius: 9, padding: '14px 34px', fontSize: 16, fontWeight: 700, cursor: 'pointer', marginRight: 12 }}>
            Start free
          </button>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
              {seoLinks.map(({ to, label }) => (
                <Link key={to} to={to} style={{ background: B.white, border: `1px solid ${B.border}`, borderRadius: 20, padding: '7px 16px', fontSize: 13, fontWeight: 600, color: B.blue, textDecoration: 'none' }}>{label} →</Link>
              ))}
            </div>
          </div>
        </section>
      ...
      <section style={{ padding: '72px 20px', background: B.navy, textAlign: 'center' }}>
        ...
        <button onClick={handleSignup}
          style={{ background: B.blue, color: '#fff', border: 'none', borderRadius: 9, padding: '14px 36px', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
          Start free today
        </button>
      </section>
